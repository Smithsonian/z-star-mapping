...
author: "James R Holmquist, Kevin Buffington, Meagan Gonnea, Kevin Kroeger, Lisamarie Windham-Myers"
...
Tidal wetlands provide myriad ecosystem services and have uncertain vulnerability or resilience to rising sea-levels. We have identified a need for mapping wetland elevation zones and vulnerability proxies at a national scale. However wetlands of the contiguous united states (CONUS) are geographically diverse with differing elevation gradients, and tidal ranges, making broad geographic or intercoastal comparisons of wetland processes and vulnerability difficult.  We have been able to overcome some barriers to intercomparabitiy by adopting relative tidal elevation (`Z*`), a metric which collapses elevation and tidal range into a single, more universal, subhabitat and vulenerability metric. We created a 30 x 30 m map of `Z*` spanning the majority of potentially tidal wetlands in the contiguous United States (CONUS). In our CONUS-wide analyses we find that high marshes -- marshes occuring at elevations higher than the mean high water line -- are much more widespread than has been previously emphasized in the modeling literature. We demonstrate how to propegate uncertainty in `Z*` arising from elevation vertical mapping error and tidal datum transformations. At the national-scale we additionally mapped a north to south trend in `Z*` uncertainty; areas with narrower tidal ranges have higher uncertainty than areas with wider tidal ranges. Light detection and ranging (LiDAR) random error is the largest contribution to total `Z*` uncertainty at the scale of the individual pixel. Tidal wetland vegetation interfearence with LiDAR limits the utility of these national scale maps for site-specifc applications. However, we were able to create more reliable maps at a subset of 5 well studied _sentinel sites_, improving mapping `Z*` accuracy between # and # percent. While we were able to quantify value added by the Lidar Elevation Adjustment with NDVI (LEAN) algorithm, the most promising pracitice we identified for further improving wetland process and vulnerabiltiy mapping was not in the reanalysis analysis of pre-processed LiDAR products, but with the LiDAR data collection itself. We reccomend future coastal mapping studies prioritize collecting LiDAR data during leaf-off conditions when and where possible, and that additional investments into DEM precisions be prioritized in microtidal systems, to substaintially imrpove assessments of marsh subhabitat distributions and vulnerability to sea-level rise.
